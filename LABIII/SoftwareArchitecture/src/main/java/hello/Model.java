...
public class Model {
	...
	public List<Carro> buscarPorMarca(String marca) {
		List<Carro> carrosEncontradosPorMarca = new LinkedList<Carro>();
		
		for(Carro carro:carros) {
			if(carro.getEspc().getMarca().equals(marca)) carrosEncontradosPorMarca.add(carro);
		}
		
		return carrosEncontradosPorMarca;
	}
}
